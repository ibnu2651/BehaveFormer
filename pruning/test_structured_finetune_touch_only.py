...
model_path = f"/home/i/ibnu2651/BehaveFormer/pruning/prune_structured_touch_only_{args.config}_last.pt"
...
with torch.no_grad():
    for batch in test_dataloader:
        # touch-only dataset may return either:
        # 1) a single tensor
        # 2) a tuple/list where batch[0] is the touch tensor

        if isinstance(batch, (list, tuple)):
            behave_inputs = batch[0]
        else:
            behave_inputs = batch

        print(behave_inputs.shape)

        behave_inputs = behave_inputs.to(device).float()
        outputs = model(behave_inputs)
